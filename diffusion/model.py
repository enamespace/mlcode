...
class SinPositionEmbeddings(nn.Module):
    ...
    def forward(self, time):
        device = time.device
        half_dim = self.dim // 2
        embeddings = math.log(100000) / (half_dim - 1)
...
class WeightStandardizedConv2d(nn.Conv2d):
    def forward(self, x):
        eps = 1e-5 if x.type == torch.float32 else 1e-3
...
class LinearAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # 保证q,k 非负则内积非负 https://zhuanlan.zhihu.com/p/157490738
        q = q.softmax(dim=-2)
        k = k.softmax(dim=-1)
...
class Unet(nn.Module):
    ...
    def forward(self, x, time, x_self_cond=False):
